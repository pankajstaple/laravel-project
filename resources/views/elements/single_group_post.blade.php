<div class="card">
  ...
  <div>
@if($type=='user_post')
<a href="{{url('/profile',Auth::user()->user_code)}}"> <img src="{{config('constants.user_post_image_path').'/'.$post->post_image}}" class="img-fluid"></a>
@else
    <a href="{{url('/profile',Auth::user()->user_code)}}"><img src="{{config('constants.group_post_image_path').'/'.$post->post_image}}" class="img-fluid"></a>
@endif
  </div>
  @endif
  <ul class="list-inline border-top border-bottom py-2 px-3 mb-0">
    <li class="list-inline-item">
      <a href="javascript:;" class="like-post" data-post="{{base64_encode($post->id)}}" data-group="{{base64_encode($post->group_id)}}"> 
  ...
  <div class="add_comment py-2 px-3">
    <div class="form-row align-items-center comment-box">
      <div class="col-auto">
        @if(Auth::check() && (Auth::user()->profile_image != ''))
         <a href="{{url('/profile',Auth::user()->user_code)}}"> <div class="avtar avtar-md rounded-circle" style="background-image: url({{ asset('profile_image/'.Auth::user()->profile_image)}});"></div></a>
        @else
          <a href="{{url('/profile',Auth::user()->user_code)}}"><div class="avtar avtar-md rounded-circle" style="background-image: url({{ asset('fronttheme/images/admin-avatar.png')}});"></div></a>
        @endif
      </div>
      <div class="col">
      ...
      <div class="col-auto">
        <button type="button" class="btn btn-yellow post-comment" data-post="{{base64_encode($post->id)}}" data-group="{{base64_encode($post->group_id)}}">Post</button>
      </div>
    </div>
  </div>
  @endif                            
